...
static zend_brk_cont_element *get_next_brk_cont_element(void) {
    ...
    CG(context).brk_cont_array = erealloc(CG(context).brk_cont_array,
                                          sizeof(zend_brk_cont_element) * CG(context).last_brk_cont);
...
static void
zend_end_live_range(zend_op_array *op_array, uint32_t offset, uint32_t end, uint32_t kind, uint32_t var) /* {{{ */
{
...
zend_string *
zend_concat3(char *str1, size_t str1_len, char *str2, size_t str2_len, char *str3, size_t str3_len) /* {{{ */
{
...
zend_string *zend_resolve_class_name(zend_string *name, uint32_t type) /* {{{ */
{
    ...
    if (FC(imports)) {
        ...
        if (compound) {
            ...
            if (import_name) {
                return zend_concat_names(
                        ZSTR_VAL(import_name), ZSTR_LEN(import_name), ZSTR_VAL(name) + len + 1,
                        ZSTR_LEN(name) - len - 1);
            }
        } else {
...
ZEND_API int do_bind_function(const zend_op_array *op_array, const zend_op *opline, HashTable *function_table,
...
ZEND_API zend_class_entry *do_bind_class(const zend_op_array *op_array, const zend_op *opline, HashTable *class_table,
                                         zend_bool compile_time) /* {{{ */
{
    zend_class_entry * ce;
    zval * lcname, *rtd_key, *zv;
    ...
    if (zend_hash_add_ptr(class_table, Z_STR_P(lcname), ce) == NULL) {
        ...
        if (!compile_time) {
			 ...
			 * approach to work.
			 */
            zend_error_noreturn(E_COMPILE_ERROR, "Cannot declare %s %s, because the name is already in use",
...
do_bind_inherited_class(const zend_op_array *op_array, const zend_op *opline, HashTable *class_table,
                        zend_class_entry *parent_ce, zend_bool compile_time) /* {{{ */
{
    ...
    if (!zv) {
        if (!compile_time) {
			 ...
			 * approach to work.
			 */
            zend_error_noreturn(E_COMPILE_ERROR, "Cannot declare  %s, because the name is already in use",
                                zend_get_object_type(Z_OBJCE_P(lcname)));
        }
        return NULL;
    }
...
ZEND_API void
zend_do_delayed_early_binding(const zend_op_array *op_array, uint32_t first_early_binding_opline) /* {{{ */
{
...
zend_mangle_property_name(const char *src1, size_t src1_length, const char *src2, size_t src2_length,
                          int internal) /* {{{ */
{
...
ZEND_API int zend_unmangle_property_name_ex(const zend_string *name, const char **class_name, const char **prop_name,
                                            size_t *prop_len) /* {{{ */
{
...
static zend_bool zend_try_compile_const_expr_resolve_class_name(zval *zv, zend_ast *class_ast, zend_ast *name_ast,
                                                                zend_bool constant) /* {{{ */
{
    uint32_t fetch_type;
    zval * class_name;
...
static zend_bool zend_try_ct_eval_class_const(zval *zv, zend_string *class_name, zend_string *name) /* {{{ */
{
    ...
    } else if (fetch_type == ZEND_FETCH_CLASS_DEFAULT &&
...
int
zend_register_auto_global(zend_string *name, zend_bool jit, zend_auto_global_callback auto_global_callback) /* {{{ */
{
    zend_auto_global auto_global;
    int retval;
    ...
    retval = zend_hash_add_mem(CG(auto_globals), auto_global.name, &auto_global, sizeof(zend_auto_global)) != NULL
             ? SUCCESS : FAILURE;
...
ZEND_API void zend_initialize_class_data(zend_class_entry *ce, zend_bool nullify_handlers) /* {{{ */
{
    ...
    zend_hash_init_ex(&ce->properties_info, 8, NULL, (persistent_hashes ? zend_destroy_property_info_internal : NULL),
                      persistent_hashes, 0);
        ...
        znode *result, zend_ast *ast, znode *expr_node, zend_bool old_style) /* {{{ */
{
    ...
    for (i = 0; i < list->children; ++i) {
        ...
        opline = zend_emit_op(&fetch_result,
                              elem_ast->attr ? (expr_node->op_type == IS_CV ? ZEND_FETCH_DIM_W : ZEND_FETCH_LIST_W)
    ...
    if (fbc) {
        if (fbc->type == ZEND_INTERNAL_FUNCTION) {
            if (init_op->opcode == ZEND_INIT_FCALL && !zend_execute_internal) {
                if (!(fbc->common.fn_flags & (ZEND_ACC_ABSTRACT | ZEND_ACC_DEPRECATED | ZEND_ACC_HAS_TYPE_HINTS |
...
void zend_compile_dynamic_call(znode *result, znode *name_node, zend_ast *args_ast) /* {{{ */
{
    if (name_node->op_type == IS_CONST && Z_TYPE(name_node->u.constant) == IS_STRING) {
        ...
        if ((colon = zend_memrchr(ZSTR_VAL(str), ':', ZSTR_LEN(str))) != NULL && colon > ZSTR_VAL(str) &&
            *(colon - 1) == ':') {
...
static void zend_compile_static_var_common(zend_ast *var_ast, zval *value, uint32_t by_ref) /* {{{ */
{
    ...
    opline->extended_value =
            (uint32_t) ((char *) value - (char *) CG(active_op_array)->static_variables->arData) | by_ref;
}
...
void zend_compile_params(zend_ast *ast, zend_ast *return_type_ast) /* {{{ */
{
    ...
    for (i = 0; i < list->children; ++i) {
        ...
        if (is_variadic) {
            ...
            CG(compiler_options) |=
                    ZEND_COMPILE_NO_CONSTANT_SUBSTITUTION | ZEND_COMPILE_NO_PERSISTENT_CONSTANT_SUBSTITUTION;
            ...
            zend_bool has_null_default = default_ast
                                         && (Z_TYPE(default_node.u.constant) == IS_NULL
                                             || (Z_TYPE(default_node.u.constant) == IS_CONSTANT_AST
                                                 ...
                                                 && strcasecmp(
                    ZSTR_VAL(zend_ast_get_constant_name(Z_ASTVAL(default_node.u.constant))), "NULL") == 0));
                ...
                if (default_ast && !has_null_default && Z_TYPE(default_node.u.constant) != IS_CONSTANT_AST) {
                    if (ZEND_TYPE_IS_CLASS(arg_info->type)) {
                        ...
                        switch (ZEND_TYPE_CODE(arg_info->type)) {
                            case IS_DOUBLE:
                                if (Z_TYPE(default_node.u.constant) != IS_DOUBLE &&
                            ...
                            default:
                                if (!ZEND_SAME_FAKE_TYPE(ZEND_TYPE_CODE(arg_info->type),
                                                         Z_TYPE(default_node.u.constant))) {
                                    zend_error_noreturn(E_COMPILE_ERROR, "Default value for parameters "
...
static void zend_compile_trait_precedence(zend_ast *ast) /* {{{ */
{
    ...
    zend_trait_precedence *precedence = emalloc(
            sizeof(zend_trait_precedence) + (insteadof_list->children - 1) * sizeof(zend_string *));
    ...
    if (extends_ast) {
        ...
        extends_const = zend_add_class_name_literal(CG(active_op_array),
                                                    zend_resolve_class_name(extends_name,
                                                                            extends_ast->kind == ZEND_AST_ZVAL
                                                                            ? extends_ast->attr : ZEND_NAME_FQ));
...
static void zend_check_already_in_use(uint32_t type, zend_string *old_name, zend_string *new_name,
                                      zend_string *check_name) /* {{{ */
{
    ...
    zend_error_noreturn(E_COMPILE_ERROR, "Cannot use%s %s as %s because the name "
                                         "is already in use", zend_get_use_type_str(type), ZSTR_VAL(old_name),
                        ZSTR_VAL(new_name));
}
    ...
    for (i = 0; i < list->children; ++i) {
        ...
        if (type == ZEND_SYMBOL_CLASS && zend_is_reserved_class_name(new_name)) {
            zend_error_noreturn(E_COMPILE_ERROR, "Cannot use %s as %s because '%s' "
                                                 "is a special class name", ZSTR_VAL(old_name), ZSTR_VAL(new_name),
                                ZSTR_VAL(new_name));
        }
...
static zend_bool zend_try_ct_eval_magic_const(zval *zv, zend_ast *ast) /* {{{ */
{
    ...
    switch (ast->attr) {
        case T_LINE: ZVAL_LONG(zv, ast->lineno);
            break;
        case T_FILE:
...
void zend_compile_const(znode *result, zend_ast *ast) /* {{{ */
{
    ...
    if (zend_string_equals_literal(resolved_name, "__COMPILER_HALT_OFFSET__") ||
        (name_ast->attr != ZEND_NAME_RELATIVE && zend_string_equals_literal(orig_name, "__COMPILER_HALT_OFFSET__"))) {
...
void zend_compile_class_const(znode *result, zend_ast *ast) /* {{{ */
{
    ...
    if (class_ast->kind == ZEND_AST_ZVAL) {
        ...
        if (const_ast->kind == ZEND_AST_ZVAL &&
            zend_try_ct_eval_class_const(&result->u.constant, resolved_name, zend_ast_get_str(const_ast))) {
            result->op_type = IS_CONST;
            zend_string_release_ex(resolved_name, 0);
            return;
        }
...
void zend_compile_top_stmt(zend_ast *ast) /* {{{ */
{
    ...
    if (ast->kind == ZEND_AST_STMT_LIST) {
        ...
        uint32_t i;
        for (i = 0; i < list->children; ++i) {
            printf("kind:%d line:%d\n", list->child[i]->kind, list->child[i]->lineno);
    ...
    switch (ast->kind) {
        ...
        case ZEND_AST_OR: {
            ...
            if (ast->kind == ZEND_AST_OR) {
                ZVAL_BOOL(&result, zend_is_true(zend_ast_get_zval(ast->child[0])) ||
                                   zend_is_true(zend_ast_get_zval(ast->child[1])));
            } else {
                ZVAL_BOOL(&result, zend_is_true(zend_ast_get_zval(ast->child[0])) &&
                                   zend_is_true(zend_ast_get_zval(ast->child[1])));
            }
            ...
            if (Z_TYPE_P(container) == IS_ARRAY) {
                ...
                if (Z_TYPE_P(dim) == IS_LONG) {
                    offset = Z_LVAL_P(dim);
                           ...
                           is_numeric_string(Z_STRVAL_P(dim), Z_STRLEN_P(dim), &offset, NULL, 1) != IS_LONG) {
                    return;
                }
